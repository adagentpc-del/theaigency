...
function publicUser(u){return u?(db.publicUser?db.publicUser(u):{id:String(u.id),email:u.email,credits:Number(u.credits||0),created_at:u.created_at}):null;}
...
app.post('/api/auth/login',rateLimit(15*60_000,30),async(req,res)=>{const email=String(req.body?.email||'').trim().toLowerCase(),password=req.body?.password||'',u=await db.getUserByEmail(email);if(!u||!await verifyPassword(password,u.password_hash))return res.status(401).json({error:'invalid_login'});setSession(res,u.id);res.json({user:publicUser(u)});});
app.post('/api/auth/logout',(req,res)=>{clearSession(res);res.json({ok:true});});
app.get('/api/me',async(req,res)=>{if(!req.session)return res.json({user:null});res.json({user:publicUser(await db.getUser(req.session.sub))});});
...
app.post('/api/events',rateLimit(60_000,120),async(req,res)=>{const name=String(req.body?.event_name||'');if(!/^[a-z0-9_]{2,64}$/.test(name))return res.status(400).json({error:'invalid_event'});const raw=req.body?.metadata&&typeof req.body.metadata==='object'?JSON.stringify(req.body.metadata):'{}',meta=JSON.parse(raw.length>4000?'{}':raw);await db.recordEvent({user_id:req.session?.sub||null,event_name:name,metadata:meta});res.status(204).end();});
app.get('/api/pricing',(req,res)=>res.json({packs:CREDIT_PACKS}));
app.post('/api/exports/consume',requireUser,rateLimit(60_000,30),async(req,res)=>{if(!await db.consumeCredit(req.user.id))return res.status(402).json({error:'credits_required'});res.json({user:publicUser(await db.getUser(req.user.id))});});
...
const seo={'/png-to-vector':['PNG to Vector Converter','Convert PNG logos and artwork to scalable SVG and vector PDF files.'],'/jpg-to-vector':['JPG to Vector Converter','Convert JPG artwork into scalable vector paths for print and production.'],'/image-to-svg':['Image to SVG Converter','Turn raster artwork into a genuine SVG made from vector paths.'],'/logo-to-vector':['Logo to Vector Converter','Make your logo scalable and print-ready without Illustrator.'],'/ai-image-to-vector':['AI Image to Vector','Turn AI-generated logos and graphics into production-ready vector artwork.'],'/chatgpt-image-to-vector':['ChatGPT Image to Vector','Convert artwork created with ChatGPT into SVG and vector PDF files for printers and manufacturers.'],'/logo-for-tshirt-printing':['Logo Files for T-Shirt Printing','Prepare scalable logo artwork for apparel and T-shirt printing.'],'/logo-for-screen-printing':['Logo Files for Screen Printing','Prepare clean, reduced-color vector artwork for screen printing.'],'/logo-for-embroidery':['Logo Files for Embroidery','Prepare clean vector artwork for embroidery digitization.'],'/logo-for-sign-printing':['Logo Files for Sign Printing','Create scalable vector artwork for signs, banners and large-format graphics.'],'/png-to-svg':['PNG to SVG Converter','Convert PNG artwork into genuine SVG vector paths.']};
const indexPath=path.join(__dirname,'index.html');
function pageHtml(route){let html=fs.readFileSync(indexPath,'utf8');const data=seo[route];if(data){html=html.replace(/<title>.*?<\/title>/,`<title>${data[0]} — Vector Me</title>`).replace(/<meta name="description" content="[^"]*" \/>/,`<meta name="description" content="${data[1]}" />`).replace('</head>',`<link rel="canonical" href="${BASE_URL}${route}" /><script type="application/ld+json">${JSON.stringify({'@context':'https://schema.org','@type':'SoftwareApplication',name:'Vector Me',applicationCategory:'DesignApplication',operatingSystem:'Web',description:data[1]})}</script></head>`);}return html;}
