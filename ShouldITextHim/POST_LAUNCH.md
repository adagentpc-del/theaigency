...
The self-hosted local-model judgment engine (`RemoteAIJudgmentProvider` + `server/`) is now built and is the app's primary judgment path — the items below are follow-ups on top of it, not the backend itself:

- **Distributed rate limiting / stronger abuse mitigation** — the deployed API currently has a real, working rate limiter (`server/src/lib/rateLimiter.ts`), but it is single-instance/in-memory: running multiple replicas means each enforces its own independent limit rather than a shared one (see `server/README.md` → "Known limitations" and `SECURITY_REVIEW.md`). Add a shared store (Redis/Upstash) or Apple DeviceCheck/App Attest before this app runs multiple replicas or sees meaningful abuse, to protect inference-hardware capacity and keep latency fair across users.
...
- **Safety pattern-list refinement** — `SafetyScanner`'s deterministic pattern list can still be evaded by misspelling, unusual phrasing, or non-English text. Since it's the safety-critical layer that must work even when the AI is unavailable, improving its recall (rather than routing it through the model, which would remove the "always works offline" guarantee) is the right direction if usage data shows this matters.
- **Live semantic-quality monitoring** — `RemoteAIJudgmentProviderLiveTests.swift` and `server/scripts/benchmark.ts` exist and are ready to run against a deployed server/model (see `FOUNDER_ACTION_REQUIRED.md`), but there's no ongoing production monitoring of judgment quality (e.g. sampling real verdicts for spot review, tracking how often the local fallback or NEED MORE CONTEXT fires, re-running the benchmark periodically as models are updated). Worth adding once there's real usage to monitor.
- **Model upgrades over time** — as better small open-weight models become available, re-run `server/scripts/benchmark.ts` against them and only switch if a candidate clears the documented threshold at an equal-or-smaller size, or clears it meaningfully better at the same size. The benchmark harness exists specifically so this stays an evidence-based decision, not a preference-based one.
